...
public class Vampire extends Enemy {
    ...
    // Should set their health to 80%-90% of player's health
    double mod = ThreadLocalRandom.current().nextDouble(.20, .10);
    public void setHealth() {
        health = Math.round(player.getHealth()-(player.getHealth() * mod));
    }


    // Should set their defense to 70%-90% of Player's defense
    public void setDefense() {
        double mod = ThreadLocalRandom.current().nextDouble(.70, .90);
        // This should round the enemy defense to a whole double that is 70%-90% of the Player's defense.
        defense = Math.round(player.getDefense()-(player.getDefense()*mod));
    }


    // Should set attack to between 70%-85% of Player's attack
    public void setAttack() {
        double mod = ThreadLocalRandom.current().nextDouble(.70, .85);
